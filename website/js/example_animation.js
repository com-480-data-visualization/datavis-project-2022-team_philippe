class FillCupViz {
  constructor(svgClass, fillSvgId, grabSvgId) {
    ...
    const glass = svg
      .append("g")
      .attr("transform", "translate(-31.879 -105.47)")
      .style("cursor", "pointer")
      .attr("class", "basicsGlass");
    ...
    const maxH = 110, minH = 130;
    const fillToPercent = d3.scaleLinear().domain([0, 100]).range([minH, maxH]);

    const upperpart = glass
      .append("path")
    ...
    const firefoxY = function (y, ratio) {
      return (
        y -
        ratio *
          parseFloat(
    ...
    const updateColor = function () {
      currentPalette =
        currentPalette == whitePalette ? redPalette : whitePalette;
      colorTextSwitch
        .style("color", currentPalette.textColor)
    ...
    /////// GRABBING //////
    ///////////////////////
    const grabSvg = d3.select("#" + grabSvgId);
    const grabGlass = grabSvg.select(".basicsGlass");
    const grabHandle1 = grabGlass
      .append("rect")
      .attr("class", "handle")
      .attr("x", wine.attr("x"))
      .attr("y", fillToPercent(110))
      .attr("height", 20)
      .attr("width", wine.attr("width"))
      .on("click", () => {
        grabText.text("Don't warm up the wine");
    ...
    const grabHandle2 = grabGlass
      ...
      .attr("width", wine.attr("width"))
      .on("click", () => {
        grabText.text("Good job !");
        grabSvg.classed("shake", false);
        grabSvg.classed("pickup", true);
        setTimeout(() => grabSvg.classed("pickup", false), 2000);
      });

    const grabHandle3 = grabGlass
      .append("rect")
      .attr("class", "handle")
      .attr("x", wine.attr("x"))
      .attr("y", fillToPercent(-70))
      .attr("height", 4)
      .attr("width", wine.attr("width"))
      .on("click", () => {
        grabText.text("It's gonna fall...");
        grabSvg.classed("pickup", false);
        grabSvg.classed("shake", true);
